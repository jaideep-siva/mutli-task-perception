# Legacy Detection Baseline: Low-Data Training Pipeline

This document preserves the older Faster R-CNN detection training pipeline. It is useful for detector-only reference work, but it is not the canonical multitask training path. For the current multitask path, use `README.md`, `configs/multitask/multitask_resnet18.yaml`, and `scripts/train.py`.
...
```bash
pip install torch torchvision albumentations torchmetrics tensorboard matplotlib numpy pillow pyyaml wandb
```
...
If you want checkpoints and metrics stored outside Git, W&B is a better home for them.
...
2. Enable W&B in `configs/train/detector_demo.yaml`:
...
When enabled, the script uploads:
- `outputs/checkpoints/best.pth` as a `model` artifact
- `outputs/checkpoints/last.pth` as a `model` artifact
- `outputs/checkpoints/metrics.csv` and `outputs/metrics.jsonl` as `metrics` artifacts
- Per-epoch scalar metrics to the W&B run dashboard
...
For debugging, check the training logs and TensorBoard outputs first.
